...
class MLP(nn.Module):
    """Small two-layer MLP with LeakyReLU and dropout."""
...
class PACT(nn.Module):
    ...
    Core flow:
      1. GraphSAGE encodes node tokens per timestep.
    ...
    def _encode_nodes_one_time(self, x_t: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        h = x_t
        for conv in self.convs:
            h = conv(h, edge_index)
            h = self.act(h)
    ...
    def forward(self, batch, station_feat: torch.Tensor | None = None, return_aux: bool = False):
        ...
        for t in range(steps):
            node_tokens = self._encode_nodes_one_time(x_hist[:, t, :], edge_index)
